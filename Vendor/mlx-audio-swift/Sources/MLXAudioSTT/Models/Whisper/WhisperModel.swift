...
public final class WhisperModel: Module, STTGenerationModel {
    ...
    public func generate(
        ...
        while offsetSamples < totalSamples {
            let end = min(offsetSamples + windowSamples, totalSamples)
            let slice = mono[offsetSamples..<end]
            let offsetSeconds = Double(offsetSamples) / Double(sampleRate)
            let remaining = Double(totalSamples - offsetSamples) / Double(sampleRate)
            if generationParameters.verbose {
                print("[Whisper] window \(windowIndex + 1) \(String(format: "%.1f", offsetSeconds))s..\(String(format: "%.1f", offsetSeconds + remaining))s")
            }
            let (tokens, promptTokens, generationTokens, lang) = transcribeChunk(
                audio: slice,
            ...
            let parsed = WhisperTimestampDecode.decodeWindow(
                tokens: tokens,
                timestampBeginId: tokenizer.timestampBeginId,
                remainingDuration: remaining,
                decodeText: { tokenizer.decode(tokens: $0) }
            )
            ...
            let advance = max(1, Int((parsed.seekAdvance * Double(sampleRate)).rounded()))
            let next = offsetSamples + max(advance, 1)
            if next <= offsetSamples { break }
            offsetSamples = min(totalSamples, next)
        }
    ...
    private func transcribeChunk(
    ...
    ) -> (tokens: [Int], promptTokens: Int, generationTokens: Int, language: String?) {
        guard let tokenizer else {
            fatalError("WhisperTokenizer not loaded — call fromPretrained / fromDirectory before generate.")
        }
        ...
        for step in 0..<maxTokens {
            ...
            stepLogits = WhisperTimestampDecode.applyTimestampRules(
                logits: stepLogits,
                generated: generated,
                timestampBeginId: tokenizer.timestampBeginId,
                noTimestampsId: tokenizer.noTimestampsId,

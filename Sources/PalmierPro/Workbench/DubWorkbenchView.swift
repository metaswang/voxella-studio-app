...
struct VideoEditorLauncherView: View {
    ...
    var body: some View {
        VStack(spacing: 18) {
            ...
            Text("The complete timeline editor runs locally. Online generation and cloud transcription are not part of this edition.")
                .multilineTextAlignment(.center)
                .foregroundStyle(AppTheme.Text.tertiaryColor)
                .frame(maxWidth: 520)
            if appState.activeProject != nil {
                Button("Resume Project") { appState.resumeEditor() }
                    .buttonStyle(.borderedProminent)
                HStack(spacing: 12) {
                    Button("New Project…") { AppState.shared.createProjectInteractively() }
                        .buttonStyle(.bordered)
                    Button("Open Project…") { AppState.shared.openProjectFromPanel() }
                        .buttonStyle(.bordered)
                }
            } else {
                HStack(spacing: 12) {
            ...
            Text("Legacy .palmier projects can be opened and saved as .voxella projects.")
                .font(.system(size: AppTheme.FontSize.xs))
                .foregroundStyle(AppTheme.Text.mutedColor)
        }
        .frame(maxWidth: .infinity, maxHeight: .infinity)

// Derived from mlx-swift-lm's Qwen35 implementation; adds a pre-LM-head embedding path.
...
private enum WeMMQwen35VLError: Error {
...
private let ropeDeltasKey = LMOutput.Key<MLXArray>(
    "qwen35.ropeDeltas")
private let embeddingHiddenStateKey = LMOutput.Key<Bool>(
    "wemm.embeddingHiddenState")

// MARK: - Configuration

public struct WeMMQwen35Configuration: Codable, Sendable {
...
enum WeMMQwen35Language {
    ...
    static func applyMultimodalRotaryPosEmb(
        ...
        let qEmbedded = (qRot * cos) + (WeMMQwen3VLVision.rotateHalf(qRot) * sin)
        let kEmbedded = (kRot * cos) + (WeMMQwen3VLVision.rotateHalf(kRot) * sin)
    ...
    final class Attention: Module {
        ...
        init(_ args: WeMMQwen35Configuration.TextConfiguration) {
    ...
    final class DecoderLayer: Module {
        ...
        init(_ args: WeMMQwen35Configuration.TextConfiguration, layerIdx: Int) {
    ...
    final class LanguageModel: Module {
        ...
        let config: WeMMQwen35Configuration
        let textConfig: WeMMQwen35Configuration.TextConfiguration
        let modelType: String
        let kvHeads: [Int]

        init(_ config: WeMMQwen35Configuration) {
            self.config = config
        ...
        func callAsFunction(
            ...
            let returnHiddenState = state[embeddingHiddenStateKey] ?? false
            ...
            if positionIds == nil && (ropeMask == nil || ropeMask?.ndim == 2) {
                    ...
                    || cache == nil
                {
                    if let precomputedPositionIds {
                        ...
                        let (computed, deltas) = WeMMQwen35Language.getRopeIndex(
                            inputIds: inputs,
...
public class WeMMQwen35EmbeddingModel: Module, VLMModel {
    @ModuleInfo(key: "vision_tower") private var visionModel: WeMMQwen3VLVision.VisionModel
    @ModuleInfo(key: "language_model") fileprivate var languageModel: WeMMQwen35Language.LanguageModel

    public let config: WeMMQwen35Configuration

    public init(_ config: WeMMQwen35Configuration) {
        self.config = config
        _visionModel.wrappedValue = WeMMQwen3VLVision.VisionModel(config.visionConfiguration)
        _languageModel.wrappedValue = WeMMQwen35Language.LanguageModel(config)
        super.init()
    }
    ...
    private func mergeInputIdsWithImageFeatures(
        ...
        guard nImageMaskElements == imageFeatureSize else {
            throw WeMMQwen35VLError.featureTokenMismatch(expected: nImageTokens, actual: nImageFeatures)
        }
    ...
    public func embedding(_ input: LMInput) throws -> MLXArray {
        ...
        state[embeddingHiddenStateKey] = true
...
extension WeMMQwen35EmbeddingModel {
